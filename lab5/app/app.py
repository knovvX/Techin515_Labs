...
def predict():
    try:
        ...
        # input_array = np.array(data, dtype=np.float32).reshape(1, 100, 3)
        ...
        #     }), 400
        # input_array = np.array(data, dtype=np.float32).reshape(1, 100, 3)  # ← FIXED
        input_array = np.array(data, dtype=np.float32)

        if input_array.shape != (300,):
            return jsonify({"error": f"Expected 300 elements, got shape {input_array.shape}"}), 400

        input_array = input_array.reshape(1, 300)  # ✅ model expects (None, 300)
        ...
        print("Input shape is valid:", input_array.shape)    
        # Make prediction
